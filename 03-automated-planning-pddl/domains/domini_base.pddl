(define (domain hotel)
  ...
  (:action assignar_reserva
      ...
      :effect (and
                ...
                ;penelització per capacitat Cost = X * (capacitat_habitacio - persones_reserva)
                (increase (total-cost) 
                    (* 500 (- (capacitat_habitacio ?h) (persones_reserva ?r)))
                )

                ;augmenta el cost si ha d'obrir una nova habitació
                (when (nova_habitacio ?h)
                    (and 
                        (increase (total-cost) 10000)
                        (not (nova_habitacio ?h))
                    )
                )
              )
  )
